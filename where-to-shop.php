...
<head>
  ...
  <meta property="og:title" content="KL The Guide - <?php echo $tile2_title2 ?>" />
  ...
  <meta property="twitter:title" content="KL The Guide - <?php echo $tile2_title2 ?>" />
      ...
      <div class="container" data-aos="fade-up">
        <div class="tab-content">
          <div class="tab-pane active show " id="tab-1">
            ...
            <!-- Carousel Wrapper -->
            <div id="wtsCarousel" class="carousel slide" data-bs-ride="false" data-bs-interval="false">
              <!-- Carousel Indicators (Optional) -->
              <div class="carousel-indicators">
                <?php
                // First, get the total number of items to create indicators
                $query_count = "SELECT COUNT(*) as total FROM place_shop";
                ...
                for ($i = 0; $i < $total_slides; $i++) {
                  ...
                  echo '<button type="button" data-bs-target="#wtsCarousel" data-bs-slide-to="' . $i . '" class="' . $active_class . '" aria-label="Slide ' . ($i + 1) . '"></button>';
                }
                ?>
              </div>
                ...
                ?>
                  ...
                  <div class="col-md-6"> <!-- Each card takes half the width on medium screens and up -->
                    <div class="card h-100 border-0 shadow-sm rounded-3">
                      <img src="assets/img/place_to_shop/<?php echo $row['place_shop_image'] ?>"
                        alt="<?php echo urldecode($row['place_shop_title']) ?>"
                        class="card-img-top img-fluid rounded-top-4" loading="lazy">
                      <div class="card-body d-flex flex-column">
                        <h4 class="card-title mb-3"><?php echo urldecode($row['place_shop_title']) ?></h4>
                        <p class="card-text flex-grow-1"><?php echo urldecode($row['place_shop_content']) ?></p>

                        <?php if ($row['place_shop_location']) { ?>
                          <p class="mb-2">
                            <i class="bi bi-geo-alt-fill" style="color: black;"></i>
                            <a href="<?php echo $row['place_shop_locationurl'] ?>" target="_blank">
                        ...
                        <?php if ($row['place_shop_phone']) { ?>
                          <p class="mb-2">
                            <i class="bi bi-telephone-fill" style="color: black;"></i>
                            <?php echo urldecode($row['place_shop_phone']) ?>
                          </p>
                        <?php } ?>

                        <?php if ($row['place_shop_website']) { ?>
                          <p class="mb-2">
                            <i class="bi bi-globe" style="color: black;"></i>
                            <a href="<?php echo $row['place_shop_website'] ?>" target="_blank">
                              <?php echo urldecode($row['place_shop_website']) ?>
                            </a>
                          </p>
                        <?php } ?>
                      </div>
              ...
              <!-- Carousel Controls -->
              <button class="carousel-control-prev" type="button" data-bs-target="#wtsCarousel" data-bs-slide="prev">
                <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                <span class="visually-hidden">Previous</span>
              </button>
              <button class="carousel-control-next" type="button" data-bs-target="#wtsCarousel" data-bs-slide="next">
  ...
  <script>
    $(document).ready(function() {
      ...
      document.addEventListener('DOMContentLoaded', function() {
        const carouselElement = document.getElementById('wtsCarousel');
