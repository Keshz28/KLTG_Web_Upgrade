...
if (isset($_POST['upload_eklhs'])) {
    ...
    $uploaded_file = $_FILES["fileToUploadeklhs"]; // Get the file array

    // Validate file upload
    if (!isset($uploaded_file['error']) || $uploaded_file['error'] !== UPLOAD_ERR_OK) {
        error_log("File upload error for explorekl_hs: " . $uploaded_file['error']);
        ...
        if ($check === false) {
            ...
            $check = getimagesize($uploaded_file["tmp_name"]);
            if ($check === false) {
                error_log("File is not an image for explorekl_hs: " . $uploaded_file['name']);
                $_SESSION['alert_type'] = 'error';
                $_SESSION['alert_msg'] = "File is not an image.";
            } else {
                // Check file size (e.g., 5MB limit)
                if ($uploaded_file["size"] > 5000000) { // 5MB in bytes
                    error_log("File too large for explorekl_hs: " . $uploaded_file['name']);
                    $_SESSION['alert_type'] = 'error';
                    $_SESSION['alert_msg'] = "Sorry, your file is too large.";
                } else {
                    // Allow certain file formats
                    if ($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif") {
                        error_log("Invalid file type for explorekl_hs: " . $uploaded_file['name']);
                        $_SESSION['alert_type'] = 'error';
                        $_SESSION['alert_msg'] = "Sorry, only JPG, JPEG, PNG & GIF files are allowed.";
                    } else {
                        // Generate unique filename
                        $newfilename = time() . '_' . basename($uploaded_file["name"]);
                        $target_dir = "../assets/img/explorekl/hs/"; // Adjust path if necessary
                        $target_file = $target_dir . $newfilename;

                        // Attempt to move uploaded file
                        if (!move_uploaded_file($uploaded_file["tmp_name"], $target_file)) {
                            error_log("Could not move uploaded file for explorekl_hs: " . $target_file);
                            $_SESSION['alert_type'] = 'error';
                            $_SESSION['alert_msg'] = "Sorry, there was an error uploading your file.";
                        } else {
                            // Calculate the new order (assuming it's the highest + 1)
                            $order_query = "SELECT MAX(explorekl_hs_order) AS max_order FROM explorekl_hs";
                            $order_result = mysqli_query($db, $order_query);
                            if (!$order_result) {
                                error_log("Database query failed for max order in explorekl_hs: " . mysqli_error($db));
                                $_SESSION['alert_type'] = 'error';
                                ...
                                // Insert data into DB
                                $query = "INSERT INTO explorekl_hs (explorekl_hs_title,explorekl_hs_content,explorekl_hs_location,explorekl_hs_locationurl,explorekl_hs_image,explorekl_hs_hours,explorekl_hs_phone,explorekl_hs_order,explorekl_hs_mapcoords)
                                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
                                ...
                                if ($stmt) {
                                    ...
                                    if ($insert_result) {
                                        ...
                                        $_SESSION['alert_msg'] = "Added New explorekl_hs";
                                    } else {
                                        error_log("Database insertion failed for explorekl_hs: " . mysqli_error($db));
                                        $_SESSION['alert_type'] = 'error';
                                        $_SESSION['alert_msg'] = "Database insertion failed: " . mysqli_error($db);
                                    }
                                    mysqli_stmt_close($stmt);
                                } else {
                                    error_log("Database prepare failed for explorekl_hs: " . mysqli_error($db));
                                    $_SESSION['alert_type'] = 'error';
    ...
    $redirect_url = $_SERVER['PHP_SELF'] . "?addnew#table9"; // Adjust hash if necessary for HS table
...
if (isset($_GET['orderupeklhs'])) {
    $order = (int)$_GET['orderupeklhs'];
    $explorekl_hs_id = (int)$_GET['explorekl_hs_id'];
    $order2 = $order + 1;

    $query = "UPDATE explorekl_hs SET explorekl_hs_order= ? WHERE explorekl_hs_id=?";
    $stmt = mysqli_prepare($db, $query);
    if ($stmt) {
        mysqli_stmt_bind_param($stmt, "ii", $order2, $explorekl_hs_id);
        ...
        if ($update_result) {
            ...
            error_log("Database update order up failed for explorekl_hs ID $explorekl_hs_id: " . mysqli_error($db));
            $_SESSION['alert_type'] = 'error';
            $_SESSION['alert_msg'] = "Database update failed: " . mysqli_error($db);
        }
        mysqli_stmt_close($stmt);
    } else {
        error_log("Database prepare failed for update order explorekl_hs: " . mysqli_error($db));
        $_SESSION['alert_type'] = 'error';
...
if (isset($_GET['orderdowneklhs'])) {
    ...
    if ($stmt) {
        ...
        if ($update_result) {
            ...
            error_log("Database update order down failed for explorekl_hs ID $explorekl_hs_id: " . mysqli_error($db));
            $_SESSION['alert_type'] = 'error';
            $_SESSION['alert_msg'] = "Database update failed: " . mysqli_error($db);
        }
        mysqli_stmt_close($stmt);
...
if (isset($_POST['deleteeklhs'])) {
    $id = (int)$_POST['eklhsid']; // Cast to integer for security
    $filename = $_POST['imagenameeklhs'];

    $query = "DELETE FROM explorekl_hs WHERE explorekl_hs_id=?";
    $stmt = mysqli_prepare($db, $query);
    if ($stmt) {
        ...
        if ($delete_result) {
            if (mysqli_stmt_affected_rows($stmt) > 0) {
                ...
                $file_path = "../assets/img/explorekl/hs/" . $filename; // Adjust path if necessary
                if (file_exists($file_path) && !unlink($file_path)) {
                    error_log("Could not delete file for explorekl_hs: " . $file_path);
            ...
            // DB deletion failed
            error_log("Database deletion failed for explorekl_hs ID $id: " . mysqli_error($db));
            $_SESSION['alert_type'] = 'error';
            $_SESSION['alert_msg'] = "Database deletion failed: " . mysqli_error($db);
        }
        mysqli_stmt_close($stmt);
    } else {
        error_log("Database prepare failed for delete explorekl_hs: " . mysqli_error($db));
        $_SESSION['alert_type'] = 'error';
...
if (isset($_POST['editeklhs'])) {
    ...
    $id = (int)$_POST['eklhsid']; // Cast to integer
    $filename = $_POST['imagenameeklhs'];

    // Check if a new file was uploaded
    $new_filename = $filename; // Default to existing filename
    $uploaded_file = $_FILES["fileToUploadeklhs"];
    if (isset($uploaded_file) && $uploaded_file['error'] == UPLOAD_ERR_OK) {
        ...
        $check = getimagesize($uploaded_file["tmp_name"]);
        if ($check === false) {
            error_log("New file is not an image for explorekl_hs ID $id: " . $uploaded_file['name']);
            $_SESSION['alert_type'] = 'error';
            $_SESSION['alert_msg'] = "New file is not an image.";
        } else {
            if ($uploaded_file["size"] > 5000000) {
                error_log("New file too large for explorekl_hs ID $id: " . $uploaded_file['name']);
                $_SESSION['alert_type'] = 'error';
                $_SESSION['alert_msg'] = "Sorry, your new file is too large.";
            } else {
                if ($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif") {
                    error_log("Invalid new file type for explorekl_hs ID $id: " . $uploaded_file['name']);
                    $_SESSION['alert_type'] = 'error';
                    ...
                    if (!move_uploaded_file($uploaded_file["tmp_name"], $target_file)) {
                        error_log("Could not move new uploaded file for explorekl_hs ID $id: " . $target_file);
                        $_SESSION['alert_type'] = 'error';
                        ...
                        if ($filename && $filename !== $new_filename) {
                            $old_file_path = "../assets/img/explorekl/hs/" . $filename; // Adjust path if necessary
                            if (file_exists($old_file_path) && !unlink($old_file_path)) {
                                error_log("Could not delete old file for explorekl_hs ID $id: " . $old_file_path);
    ...
    $query = "UPDATE explorekl_hs SET
              explorekl_hs_title=?,
              explorekl_hs_order=?,
              explorekl_hs_location=?,
              explorekl_hs_locationurl=?,
              explorekl_hs_content=?,
              explorekl_hs_hours=?,
              explorekl_hs_phone=?,
              explorekl_hs_image=?,
              explorekl_hs_mapcoords=?
              WHERE explorekl_hs_id=?";
    $stmt = mysqli_prepare($db, $query);
    if ($stmt) {
        ...
        if ($update_result) {
            ...
            error_log("Database update failed for explorekl_hs ID $id: " . mysqli_error($db));
            $_SESSION['alert_type'] = 'error';
            $_SESSION['alert_msg'] = "Database update failed: " . mysqli_error($db);
        }
        mysqli_stmt_close($stmt);
    } else {
        error_log("Database prepare failed for update explorekl_hs ID $id: " . mysqli_error($db));
        $_SESSION['alert_type'] = 'error';
        $_SESSION['alert_msg'] = "Database error preparing update query.";
    }
