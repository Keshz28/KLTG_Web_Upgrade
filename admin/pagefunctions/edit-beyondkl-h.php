...
if (isset($_POST['upload_bklh'])) {
    ...
    if (!isset($uploaded_file['error']) || $uploaded_file['error'] !== UPLOAD_ERR_OK) {
        error_log("File upload error for beyondkl_h: " . $uploaded_file['error']);
        ...
        if ($check === false) {
            ...
            $check = getimagesize($uploaded_file["tmp_name"]);
            if ($check === false) {
                error_log("File is not an image for beyondkl_h: " . $uploaded_file['name']);
                $_SESSION['alert_type'] = 'error';
                $_SESSION['alert_msg'] = "File is not an image.";
            } else {
                // Check file size (e.g., 5MB limit)
                if ($uploaded_file["size"] > 5000000) { // 5MB in bytes
                    error_log("File too large for beyondkl_h: " . $uploaded_file['name']);
                    $_SESSION['alert_type'] = 'error';
                    $_SESSION['alert_msg'] = "Sorry, your file is too large.";
                } else {
                    // Allow certain file formats
                    if ($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif") {
                        error_log("Invalid file type for beyondkl_h: " . $uploaded_file['name']);
                        $_SESSION['alert_type'] = 'error';
                        $_SESSION['alert_msg'] = "Sorry, only JPG, JPEG, PNG & GIF files are allowed.";
                    } else {
                        // Generate unique filename
                        $newfilename = time() . '_' . basename($uploaded_file["name"]);
                        $target_dir = "../assets/img/beyondkl/h/"; // Adjust path if necessary
                        $target_file = $target_dir . $newfilename;

                        // Attempt to move uploaded file
                        if (!move_uploaded_file($uploaded_file["tmp_name"], $target_file)) {
                            error_log("Could not move uploaded file for beyondkl_h: " . $target_file);
                            $_SESSION['alert_type'] = 'error';
                            $_SESSION['alert_msg'] = "Sorry, there was an error uploading your file.";
                        } else {
                            // Calculate the new order (assuming it's the highest + 1)
                            $order_query = "SELECT MAX(beyondkl_h_order) AS max_order FROM beyondkl_h";
                            $order_result = mysqli_query($db, $order_query);
                            if (!$order_result) {
                                error_log("Database query failed for max order in beyondkl_h: " . mysqli_error($db));
                                $_SESSION['alert_type'] = 'error';
                                ...
                                if ($stmt) {
                                    ...
                                    if ($insert_result) {
                                        ...
                                        $_SESSION['alert_msg'] = "Added New beyondkl_h";
                                    } else {
                                        error_log("Database insertion failed for beyondkl_h: " . mysqli_error($db));
                                        $_SESSION['alert_type'] = 'error';
                                        $_SESSION['alert_msg'] = "Database insertion failed: " . mysqli_error($db);
                                    }
                                    mysqli_stmt_close($stmt);
                                } else {
                                    error_log("Database prepare failed for beyondkl_h: " . mysqli_error($db));
                                    $_SESSION['alert_type'] = 'error';
...
if (isset($_GET['orderupbklh'])) {
    $order = (int)$_GET['orderupbklh'];
    $beyondkl_h_id = (int)$_GET['beyondkl_h_id'];
    $order2 = $order + 1;

    $query = "UPDATE beyondkl_h SET beyondkl_h_order= ? WHERE beyondkl_h_id=?";
    $stmt = mysqli_prepare($db, $query);
    if ($stmt) {
        mysqli_stmt_bind_param($stmt, "ii", $order2, $beyondkl_h_id);
        ...
        if ($update_result) {
            ...
            error_log("Database update order up failed for beyondkl_h ID $beyondkl_h_id: " . mysqli_error($db));
            $_SESSION['alert_type'] = 'error';
            $_SESSION['alert_msg'] = "Database update failed: " . mysqli_error($db);
        }
        mysqli_stmt_close($stmt);
    } else {
        error_log("Database prepare failed for update order beyondkl_h: " . mysqli_error($db));
        $_SESSION['alert_type'] = 'error';
...
if (isset($_GET['orderdownbklh'])) {
    ...
    if ($stmt) {
        ...
        if ($update_result) {
            ...
            error_log("Database update order down failed for beyondkl_h ID $beyondkl_h_id: " . mysqli_error($db));
            $_SESSION['alert_type'] = 'error';
            $_SESSION['alert_msg'] = "Database update failed: " . mysqli_error($db);
        }
        mysqli_stmt_close($stmt);
...
if (isset($_POST['deletebklh'])) {
    $id = (int)$_POST['bklhid']; // Cast to integer for security
    $filename = $_POST['imagenamebklh'];

    $query = "DELETE FROM beyondkl_h WHERE beyondkl_h_id=?";
    $stmt = mysqli_prepare($db, $query);
    if ($stmt) {
        ...
        if ($delete_result) {
            if (mysqli_stmt_affected_rows($stmt) > 0) {
                ...
                $file_path = "../assets/img/beyondkl/h/" . $filename; // Adjust path if necessary
                if (file_exists($file_path) && !unlink($file_path)) {
                    error_log("Could not delete file for beyondkl_h: " . $file_path);
            ...
            // DB deletion failed
            error_log("Database deletion failed for beyondkl_h ID $id: " . mysqli_error($db));
            $_SESSION['alert_type'] = 'error';
            $_SESSION['alert_msg'] = "Database deletion failed: " . mysqli_error($db);
        }
        mysqli_stmt_close($stmt);
    } else {
        error_log("Database prepare failed for delete beyondkl_h: " . mysqli_error($db));
        $_SESSION['alert_type'] = 'error';
...
if (isset($_POST['editbklh'])) {
    ...
    $uploaded_file = $_FILES["fileToUploadbklh"];
    if (isset($uploaded_file) && $uploaded_file['error'] == UPLOAD_ERR_OK) {
        ...
        $check = getimagesize($uploaded_file["tmp_name"]);
        if ($check === false) {
            error_log("New file is not an image for beyondkl_h ID $id: " . $uploaded_file['name']);
            $_SESSION['alert_type'] = 'error';
            $_SESSION['alert_msg'] = "New file is not an image.";
        } else {
            if ($uploaded_file["size"] > 5000000) {
                error_log("New file too large for beyondkl_h ID $id: " . $uploaded_file['name']);
                $_SESSION['alert_type'] = 'error';
                $_SESSION['alert_msg'] = "Sorry, your new file is too large.";
            } else {
                if ($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif") {
                    error_log("Invalid new file type for beyondkl_h ID $id: " . $uploaded_file['name']);
                    $_SESSION['alert_type'] = 'error';
                    ...
                    if (!move_uploaded_file($uploaded_file["tmp_name"], $target_file)) {
                        error_log("Could not move new uploaded file for beyondkl_h ID $id: " . $target_file);
                        $_SESSION['alert_type'] = 'error';
                        ...
                        if ($filename && $filename !== $new_filename) {
                            $old_file_path = "../assets/img/beyondkl/h/" . $filename; // Adjust path if necessary
                            if (file_exists($old_file_path) && !unlink($old_file_path)) {
                                error_log("Could not delete old file for beyondkl_h ID $id: " . $old_file_path);
    ...
    if ($stmt) {
        ...
        if ($update_result) {
            ...
            error_log("Database update failed for beyondkl_h ID $id: " . mysqli_error($db));
            $_SESSION['alert_type'] = 'error';
            $_SESSION['alert_msg'] = "Database update failed: " . mysqli_error($db);
        }
        mysqli_stmt_close($stmt);
    } else {
        error_log("Database prepare failed for update beyondkl_h ID $id: " . mysqli_error($db));
        $_SESSION['alert_type'] = 'error';
        $_SESSION['alert_msg'] = "Database error preparing update query.";
    }
