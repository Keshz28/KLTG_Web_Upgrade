...
if (isset($_POST['upload_bkles'])) {
    ...
    $uploaded_file = $_FILES["fileToUploadbkles"]; // Get the file array

    // Validate file upload
    if (!isset($uploaded_file['error']) || $uploaded_file['error'] !== UPLOAD_ERR_OK) {
        error_log("File upload error for beyondkl_es: " . $uploaded_file['error']);
        ...
        if ($check === false) {
            ...
            $check = getimagesize($uploaded_file["tmp_name"]);
            if ($check === false) {
                error_log("File is not an image for beyondkl_es: " . $uploaded_file['name']);
                $_SESSION['alert_type'] = 'error';
                $_SESSION['alert_msg'] = "File is not an image.";
            } else {
                // Check file size (e.g., 5MB limit)
                if ($uploaded_file["size"] > 5000000) { // 5MB in bytes
                    error_log("File too large for beyondkl_es: " . $uploaded_file['name']);
                    $_SESSION['alert_type'] = 'error';
                    $_SESSION['alert_msg'] = "Sorry, your file is too large.";
                } else {
                    // Allow certain file formats
                    if ($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif") {
                        error_log("Invalid file type for beyondkl_es: " . $uploaded_file['name']);
                        $_SESSION['alert_type'] = 'error';
                        $_SESSION['alert_msg'] = "Sorry, only JPG, JPEG, PNG & GIF files are allowed.";
                    } else {
                        // Generate unique filename
                        $newfilename = time() . '_' . basename($uploaded_file["name"]);
                        $target_dir = "../assets/img/beyondkl/es/"; // Adjust path if necessary
                        $target_file = $target_dir . $newfilename;

                        // Attempt to move uploaded file
                        if (!move_uploaded_file($uploaded_file["tmp_name"], $target_file)) {
                            error_log("Could not move uploaded file for beyondkl_es: " . $target_file);
                            $_SESSION['alert_type'] = 'error';
                            $_SESSION['alert_msg'] = "Sorry, there was an error uploading your file.";
                        } else {
                            // Calculate the new order (assuming it's the highest + 1)
                            $order_query = "SELECT MAX(beyondkl_es_order) AS max_order FROM beyondkl_es";
                            $order_result = mysqli_query($db, $order_query);
                            if (!$order_result) {
                                error_log("Database query failed for max order in beyondkl_es: " . mysqli_error($db));
                                $_SESSION['alert_type'] = 'error';
                                ...
                                // Insert data into DB
                                $query = "INSERT INTO beyondkl_es (beyondkl_es_title,beyondkl_es_content,beyondkl_es_location,beyondkl_es_locationurl,beyondkl_es_image,beyondkl_es_order,beyondkl_es_mapcoords)
                                        VALUES (?, ?, ?, ?, ?, ?, ?)";
                                ...
                                if ($stmt) {
                                    ...
                                    if ($insert_result) {
                                        ...
                                        $_SESSION['alert_msg'] = "Added New beyondkl_es";
                                    } else {
                                        error_log("Database insertion failed for beyondkl_es: " . mysqli_error($db));
                                        $_SESSION['alert_type'] = 'error';
                                        $_SESSION['alert_msg'] = "Database insertion failed: " . mysqli_error($db);
                                    }
                                    mysqli_stmt_close($stmt);
                                } else {
                                    error_log("Database prepare failed for beyondkl_es: " . mysqli_error($db));
                                    $_SESSION['alert_type'] = 'error';
    ...
    $redirect_url = $_SERVER['PHP_SELF'] . "?addnew#table_beyondkl_es"; // Adjust hash if necessary for BKLES table
...
if (isset($_GET['orderupbkles'])) {
    $order = (int)$_GET['orderupbkles'];
    $beyondkl_es_id = (int)$_GET['beyondkl_es_id'];
    $order2 = $order + 1;

    $query = "UPDATE beyondkl_es SET beyondkl_es_order= ? WHERE beyondkl_es_id=?";
    $stmt = mysqli_prepare($db, $query);
    if ($stmt) {
        mysqli_stmt_bind_param($stmt, "ii", $order2, $beyondkl_es_id);
        ...
        if ($update_result) {
            ...
            error_log("Database update order up failed for beyondkl_es ID $beyondkl_es_id: " . mysqli_error($db));
            $_SESSION['alert_type'] = 'error';
            $_SESSION['alert_msg'] = "Database update failed: " . mysqli_error($db);
        }
        mysqli_stmt_close($stmt);
    } else {
        error_log("Database prepare failed for update order beyondkl_es: " . mysqli_error($db));
        $_SESSION['alert_type'] = 'error';
...
if (isset($_GET['orderdownbkles'])) {
    ...
    if ($stmt) {
        ...
        if ($update_result) {
            ...
            error_log("Database update order down failed for beyondkl_es ID $beyondkl_es_id: " . mysqli_error($db));
            $_SESSION['alert_type'] = 'error';
            $_SESSION['alert_msg'] = "Database update failed: " . mysqli_error($db);
        }
        mysqli_stmt_close($stmt);
...
if (isset($_POST['deletebkles'])) {
    $id = (int)$_POST['bklesid']; // Cast to integer for security
    $filename = $_POST['imagenamebkles'];

    $query = "DELETE FROM beyondkl_es WHERE beyondkl_es_id=?";
    $stmt = mysqli_prepare($db, $query);
    if ($stmt) {
        ...
        if ($delete_result) {
            if (mysqli_stmt_affected_rows($stmt) > 0) {
                ...
                $file_path = "../assets/img/beyondkl/es/" . $filename; // Adjust path if necessary
                if (file_exists($file_path) && !unlink($file_path)) {
                    error_log("Could not delete file for beyondkl_es: " . $file_path);
            ...
            // DB deletion failed
            error_log("Database deletion failed for beyondkl_es ID $id: " . mysqli_error($db));
            $_SESSION['alert_type'] = 'error';
            $_SESSION['alert_msg'] = "Database deletion failed: " . mysqli_error($db);
        }
        mysqli_stmt_close($stmt);
    } else {
        error_log("Database prepare failed for delete beyondkl_es: " . mysqli_error($db));
        $_SESSION['alert_type'] = 'error';
...
if (isset($_POST['editbkles'])) {
    ...
    $id = (int)$_POST['bklesid']; // Cast to integer
    $filename = $_POST['imagenamebkles'];

    // Check if a new file was uploaded
    $new_filename = $filename; // Default to existing filename
    $uploaded_file = $_FILES["fileToUploadbkles"];
    if (isset($uploaded_file) && $uploaded_file['error'] == UPLOAD_ERR_OK) {
        ...
        $check = getimagesize($uploaded_file["tmp_name"]);
        if ($check === false) {
            error_log("New file is not an image for beyondkl_es ID $id: " . $uploaded_file['name']);
            $_SESSION['alert_type'] = 'error';
            $_SESSION['alert_msg'] = "New file is not an image.";
        } else {
            if ($uploaded_file["size"] > 5000000) {
                error_log("New file too large for beyondkl_es ID $id: " . $uploaded_file['name']);
                $_SESSION['alert_type'] = 'error';
                $_SESSION['alert_msg'] = "Sorry, your new file is too large.";
            } else {
                if ($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif") {
                    error_log("Invalid new file type for beyondkl_es ID $id: " . $uploaded_file['name']);
                    $_SESSION['alert_type'] = 'error';
                    ...
                    if (!move_uploaded_file($uploaded_file["tmp_name"], $target_file)) {
                        error_log("Could not move new uploaded file for beyondkl_es ID $id: " . $target_file);
                        $_SESSION['alert_type'] = 'error';
                        ...
                        if ($filename && $filename !== $new_filename) {
                            $old_file_path = "../assets/img/beyondkl/es/" . $filename; // Adjust path if necessary
                            if (file_exists($old_file_path) && !unlink($old_file_path)) {
                                error_log("Could not delete old file for beyondkl_es ID $id: " . $old_file_path);
    ...
    $query = "UPDATE beyondkl_es SET
              beyondkl_es_title=?,
              beyondkl_es_order=?,
              beyondkl_es_location=?,
              beyondkl_es_locationurl=?,
              beyondkl_es_content=?,
              beyondkl_es_image=?,
              beyondkl_es_mapcoords=?
              WHERE beyondkl_es_id=?";
    $stmt = mysqli_prepare($db, $query);
    if ($stmt) {
        ...
        if ($update_result) {
            ...
            error_log("Database update failed for beyondkl_es ID $id: " . mysqli_error($db));
            $_SESSION['alert_type'] = 'error';
            $_SESSION['alert_msg'] = "Database update failed: " . mysqli_error($db);
        }
        mysqli_stmt_close($stmt);
    } else {
        error_log("Database prepare failed for update beyondkl_es ID $id: " . mysqli_error($db));
        $_SESSION['alert_type'] = 'error';
        $_SESSION['alert_msg'] = "Database error preparing update query.";
    }
