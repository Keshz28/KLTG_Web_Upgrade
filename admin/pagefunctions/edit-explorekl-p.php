...
if (isset($_POST['upload_eklp'])) {
    ...
    $uploaded_file = $_FILES["fileToUploadeklp"]; // Get the file array

    // Validate file upload
    if (!isset($uploaded_file['error']) || $uploaded_file['error'] !== UPLOAD_ERR_OK) {
        error_log("File upload error for explorekl_p: " . $uploaded_file['error']);
        ...
        if ($check === false) {
            ...
            $check = getimagesize($uploaded_file["tmp_name"]);
            if ($check === false) {
                error_log("File is not an image for explorekl_p: " . $uploaded_file['name']);
                $_SESSION['alert_type'] = 'error';
                $_SESSION['alert_msg'] = "File is not an image.";
            } else {
                // Check file size (e.g., 5MB limit)
                if ($uploaded_file["size"] > 5000000) { // 5MB in bytes
                    error_log("File too large for explorekl_p: " . $uploaded_file['name']);
                    $_SESSION['alert_type'] = 'error';
                    $_SESSION['alert_msg'] = "Sorry, your file is too large.";
                } else {
                    // Allow certain file formats
                    if ($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif") {
                        error_log("Invalid file type for explorekl_p: " . $uploaded_file['name']);
                        $_SESSION['alert_type'] = 'error';
                        $_SESSION['alert_msg'] = "Sorry, only JPG, JPEG, PNG & GIF files are allowed.";
                    } else {
                        // Generate unique filename
                        $newfilename = time() . '_' . basename($uploaded_file["name"]);
                        $target_dir = "../assets/img/explorekl/p/"; // Adjust path if necessary
                        $target_file = $target_dir . $newfilename;

                        // Attempt to move uploaded file
                        if (!move_uploaded_file($uploaded_file["tmp_name"], $target_file)) {
                            error_log("Could not move uploaded file for explorekl_p: " . $target_file);
                            $_SESSION['alert_type'] = 'error';
                            $_SESSION['alert_msg'] = "Sorry, there was an error uploading your file.";
                        } else {
                            // Calculate the new order (assuming it's the highest + 1)
                            $order_query = "SELECT MAX(explorekl_p_order) AS max_order FROM explorekl_p";
                            $order_result = mysqli_query($db, $order_query);
                            if (!$order_result) {
                                error_log("Database query failed for max order in explorekl_p: " . mysqli_error($db));
                                $_SESSION['alert_type'] = 'error';
                                ...
                                // Insert data into DB
                                $query = "INSERT INTO explorekl_p (explorekl_p_title,explorekl_p_content,explorekl_p_location,explorekl_p_locationurl,explorekl_p_image,explorekl_p_hours,explorekl_p_phone,explorekl_p_order,explorekl_p_mapcoords)
                                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
                                ...
                                if ($stmt) {
                                    ...
                                    if ($insert_result) {
                                        ...
                                        $_SESSION['alert_msg'] = "Added New explorekl_p";
                                    } else {
                                        error_log("Database insertion failed for explorekl_p: " . mysqli_error($db));
                                        $_SESSION['alert_type'] = 'error';
                                        $_SESSION['alert_msg'] = "Database insertion failed: " . mysqli_error($db);
                                    }
                                    mysqli_stmt_close($stmt);
                                } else {
                                    error_log("Database prepare failed for explorekl_p: " . mysqli_error($db));
                                    $_SESSION['alert_type'] = 'error';
    ...
    $redirect_url = $_SERVER['PHP_SELF'] . "?addnew#table5"; // Adjust hash if necessary for P table
...
if (isset($_GET['orderupeklp'])) {
    $order = (int)$_GET['orderupeklp'];
    $explorekl_p_id = (int)$_GET['explorekl_p_id'];
    $order2 = $order + 1;

    $query = "UPDATE explorekl_p SET explorekl_p_order= ? WHERE explorekl_p_id=?";
    $stmt = mysqli_prepare($db, $query);
    if ($stmt) {
        mysqli_stmt_bind_param($stmt, "ii", $order2, $explorekl_p_id);
        ...
        if ($update_result) {
            ...
            error_log("Database update order up failed for explorekl_p ID $explorekl_p_id: " . mysqli_error($db));
            $_SESSION['alert_type'] = 'error';
            $_SESSION['alert_msg'] = "Database update failed: " . mysqli_error($db);
        }
        mysqli_stmt_close($stmt);
    } else {
        error_log("Database prepare failed for update order explorekl_p: " . mysqli_error($db));
        $_SESSION['alert_type'] = 'error';
...
if (isset($_GET['orderdowneklp'])) {
    ...
    if ($stmt) {
        ...
        if ($update_result) {
            ...
            error_log("Database update order down failed for explorekl_p ID $explorekl_p_id: " . mysqli_error($db));
            $_SESSION['alert_type'] = 'error';
            $_SESSION['alert_msg'] = "Database update failed: " . mysqli_error($db);
        }
        mysqli_stmt_close($stmt);
...
if (isset($_POST['deleteeklp'])) {
    $id = (int)$_POST['eklpid']; // Cast to integer for security
    $filename = $_POST['imagenameeklp'];

    $query = "DELETE FROM explorekl_p WHERE explorekl_p_id=?";
    $stmt = mysqli_prepare($db, $query);
    if ($stmt) {
        ...
        if ($delete_result) {
            if (mysqli_stmt_affected_rows($stmt) > 0) {
                ...
                $file_path = "../assets/img/explorekl/p/" . $filename; // Adjust path if necessary
                if (file_exists($file_path) && !unlink($file_path)) {
                    error_log("Could not delete file for explorekl_p: " . $file_path);
            ...
            // DB deletion failed
            error_log("Database deletion failed for explorekl_p ID $id: " . mysqli_error($db));
            $_SESSION['alert_type'] = 'error';
            $_SESSION['alert_msg'] = "Database deletion failed: " . mysqli_error($db);
        }
        mysqli_stmt_close($stmt);
    } else {
        error_log("Database prepare failed for delete explorekl_p: " . mysqli_error($db));
        $_SESSION['alert_type'] = 'error';
...
if (isset($_POST['editeklp'])) {
    ...
    $id = (int)$_POST['eklpid']; // Cast to integer
    $filename = $_POST['imagenameeklp'];

    // Check if a new file was uploaded
    $new_filename = $filename; // Default to existing filename
    $uploaded_file = $_FILES["fileToUploadeklp"];
    if (isset($uploaded_file) && $uploaded_file['error'] == UPLOAD_ERR_OK) {
        ...
        $check = getimagesize($uploaded_file["tmp_name"]);
        if ($check === false) {
            error_log("New file is not an image for explorekl_p ID $id: " . $uploaded_file['name']);
            $_SESSION['alert_type'] = 'error';
            $_SESSION['alert_msg'] = "New file is not an image.";
        } else {
            if ($uploaded_file["size"] > 5000000) {
                error_log("New file too large for explorekl_p ID $id: " . $uploaded_file['name']);
                $_SESSION['alert_type'] = 'error';
                $_SESSION['alert_msg'] = "Sorry, your new file is too large.";
            } else {
                if ($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg" && $imageFileType != "gif") {
                    error_log("Invalid new file type for explorekl_p ID $id: " . $uploaded_file['name']);
                    $_SESSION['alert_type'] = 'error';
                    ...
                    if (!move_uploaded_file($uploaded_file["tmp_name"], $target_file)) {
                        error_log("Could not move new uploaded file for explorekl_p ID $id: " . $target_file);
                        $_SESSION['alert_type'] = 'error';
                        ...
                        if ($filename && $filename !== $new_filename) {
                            $old_file_path = "../assets/img/explorekl/p/" . $filename; // Adjust path if necessary
                            if (file_exists($old_file_path) && !unlink($old_file_path)) {
                                error_log("Could not delete old file for explorekl_p ID $id: " . $old_file_path);
    ...
    $query = "UPDATE explorekl_p SET
              explorekl_p_title=?,
              explorekl_p_order=?,
              explorekl_p_location=?,
              explorekl_p_locationurl=?,
              explorekl_p_content=?,
              explorekl_p_hours=?,
              explorekl_p_phone=?,
              explorekl_p_image=?,
              explorekl_p_mapcoords=?
              WHERE explorekl_p_id=?";
    $stmt = mysqli_prepare($db, $query);
    if ($stmt) {
        ...
        if ($update_result) {
            ...
            error_log("Database update failed for explorekl_p ID $id: " . mysqli_error($db));
            $_SESSION['alert_type'] = 'error';
            $_SESSION['alert_msg'] = "Database update failed: " . mysqli_error($db);
        }
        mysqli_stmt_close($stmt);
    } else {
        error_log("Database prepare failed for update explorekl_p ID $id: " . mysqli_error($db));
        $_SESSION['alert_type'] = 'error';
        $_SESSION['alert_msg'] = "Database error preparing update query.";
    }
