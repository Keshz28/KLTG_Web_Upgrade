...
function initblog() {
  ...
  xhttp.onload = function () {
    ...
    if (data.length !== 0) {
      ...
      for (var i = 0; i < data.items.length; i++) {
        countitems(i,data.items.length)
        var date = new Date(data.items[i].published);
...
function countitems(i,total) {
  const numberofboxes = document.querySelectorAll('.read-more').length;
  setTimeout( () => {
    const spin2 = document.getElementById("spin2progress");
    const singular = 'result';
    const plural = 'results';
    if(numberofboxes >= i) {
        const resultword = (numberofboxes > 1) ? plural : singular;
        spin2.innerHTML = `Found ${numberofboxes+1} ${resultword}`;
    } 
  },300)
}
